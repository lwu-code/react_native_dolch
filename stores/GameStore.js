...
class GameStore {
  ...
  startTurn() {
    ...
    if (this.currentPlayer.name == "Garrett") {
  ...
  markCellSelected(idx) {
    const numAlreadySelected = this.selectedCells.filter(cell => cell > 0)
      .length;
  ...
  get readyForTry() {
    const numAlreadySelected = this.selectedCells.filter(cell => cell > 0)
      .length;
    const selectedHasExactlyOneEmpty =
      this.selectedCells
        .map((cell, idx) => {
